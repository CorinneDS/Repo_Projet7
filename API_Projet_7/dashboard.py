...
def render_title() -> None:
    ...
    with col1:
        ...
        st.markdown(
            "<h5 style='font-weight: normal; font-size: 24px;'>Ce tableau de bord permet d'afficher le scoring en % de chaque candidat au prêt.Le scoring s'appuie sur des sources de données variées (données comportementales, données provenant d'autres institutions financières, etc.). <br>Ce dashboard interactif permet aux chargés de relation client de pouvoir expliquer de façon la plus transparente possible les décisions d’octroi de crédit. Il affiche donc les criteres qui ont conduit à ce resultat.</h5><br>", 
